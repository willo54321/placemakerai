...
const testFeedback = [
  ...
  { category: 'concern', comment: "The developer only cares about profit, not our community. They built shoddily houses on their last project." },
  ...
  // Mixed/neutral
  { category: 'neutral', comment: "Could the developers confirm what the proposed opening hours for the construction site will be? I work night shifts and need to sleep during the day." },
  { category: 'neutral', comment: "Will there be a residents' meeting to discuss the S106 contributions? I'd like to understand what community benefits are being proposed." },
]

async function main() {
  // Get the first project (or specify a project ID)
  ...
  console.log(`Adding test feedback to project: ${project.name} (${project.id})`)

  // Add as public pins with varied locations around the project center
  ...
  for (let i = 0; i < testFeedback.length; i++) {
    ...
    console.log(`Added: ${feedback.comment.substring(0, 50)}...`)
  }

  console.log(`\nAdded ${testFeedback.length} test feedback items to project ${project.id}`)
}
