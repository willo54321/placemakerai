            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="input"
                />
                {stakeholders && stakeholders.filter(s => s.email).length > 0 && (
                  <div className="mt-3">
                    <div className="text-xs font-medium text-slate-500 mb-2">Quick add from stakeholders:</div>
                    <div className="max-h-32 overflow-y-auto border border-slate-200 rounded-lg divide-y divide-slate-100">
                      {stakeholders.filter(s => s.email).map(s => {
                        const isSelected = to.toLowerCase().includes(s.email!.toLowerCase())
                        return (
                          <button
                            key={s.id}
                            type="button"
                            onClick={() => {
                              if (isSelected) {
                                // Remove the email
                                setTo(prev => prev.split(',').map(e => e.trim()).filter(e => e.toLowerCase() !== s.email!.toLowerCase()).join(', '))
                              } else {
                                setTo(prev => prev ? `${prev}, ${s.email}` : s.email || '')
                              }
                            }}
                            className={`w-full flex items-center justify-between px-3 py-2 text-left text-sm hover:bg-slate-50 transition-colors ${isSelected ? 'bg-blue-50' : ''}`}
                          >
                            <div>
