 ...
 * submits, sees the moderation thank-you — using the actual product's
 * categories, copy and colours. All movement rides Stripe's marketing easing
 * (easeOutExpo) as real DOM animation, no video.
 */
...
const PIN = { x: 152, y: 154 } // where the pin lands, in a 460x345 viewBox

type Phase =
  | 'idle'      // map only
  | 'moving'    // cursor travelling to the spot
  | 'dropped'   // pin down, form open
  | 'category'  // cursor over the Positive chip
  | 'typing'    // comment being written
  | 'naming'    // name being written
  | 'submitting'// cursor over Submit
  | 'thanks'    // moderation thank-you card
  | 'rest'      // published pin with its comment popup
...
export default function MapPinDemo() {
  ...
  const cursor: Record<Phase, { x: number; y: number; visible: boolean }> = {
    idle: { x: 430, y: 330, visible: true },
    moving: { x: PIN.x + 4, y: PIN.y + 4, visible: true },
    dropped: { x: PIN.x + 4, y: PIN.y + 4, visible: true },
    category: { x: 338, y: 122, visible: true },
    typing: { x: 352, y: 176, visible: true },
    naming: { x: 340, y: 238, visible: true },
    submitting: { x: 386, y: 272, visible: true },
    thanks: { x: 420, y: 320, visible: false },
    rest: { x: 420, y: 320, visible: false },
  }
  ...
  return (
    ...
    >
      ...
      <svg viewBox="0 0 460 345" className="absolute inset-0 w-full h-full" aria-hidden="true">
        ...
        {/* click ripple */}
        {phase === 'dropped' && (
          <circle cx={PIN.x} cy={PIN.y} r="10" fill="none" stroke="#16A34A" strokeWidth="2" className="pin-ripple" />
        )}
      </svg>

      {/* attribution + zoom, for realism */}
      <span className="absolute bottom-1 left-1.5 text-[7px] text-slate-400">Map data ©2026</span>
      <div className="absolute bottom-3 right-3 flex flex-col rounded-md overflow-hidden shadow bg-white text-slate-500 text-sm leading-none">
        <button className="px-2 py-1.5 border-b border-slate-100" aria-hidden tabIndex={-1}>+</button>
        <button className="px-2 py-1.5" aria-hidden tabIndex={-1}>−</button>
      </div>
      ...
      <div
        className="absolute top-3 right-3 w-[200px] bg-white rounded-xl shadow-2xl p-3 origin-top-right"
        style={{
      ...
      <div
        className="absolute inset-x-6 top-1/2 -translate-y-1/2 bg-white rounded-xl shadow-2xl p-4 text-center"
        style={{
      ...
      <style>{`
        ...
        .pin-ripple { animation: pinRipple 0.7s ${EASE} both; }
